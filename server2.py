...
def get_mails_db():
    ...
    try:
        ...
        cur.execute("SELECT email FROM users WHERE disponibilità = true AND ruolo = 'analyst' ORDER BY score fetch first 2 rows only")
...
def update_dispo_db(mail):
    ...
    try:
        ...
        cur = conn.cursor()
        # Esempio di esecuzione di una query
        cur.execute("UPDATE users SET dipsonibilità = not disponibilità WHERE email = '{}'".format(mail))
